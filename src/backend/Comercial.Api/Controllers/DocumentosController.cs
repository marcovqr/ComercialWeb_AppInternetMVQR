...
public sealed class DocumentosController(ScaffoldedComercialContext db, ILogger<DocumentosController> log) : ControllerBase
{
    ...
    public async Task<IActionResult> Factura(int numero,CancellationToken ct)
    {
        ...
        var detalles=await db.factura_dets.AsNoTracking().Where(x=>x.fac_numero==numero).Select(x=>new{productoCodigo=x.pro_codigo,descripcion=x.pro_codigoNavigation.pro_descripcion,cantidad=x.fac_cantidad,precio=x.fac_precio,total=x.fac_cantidad*x.fac_precio,estado=x.fac_estado}).ToListAsync(ct);return Ok(new{cabecera=cab,detalles});
    }
    ...
    public async Task<IActionResult> ImprimirFactura(int numero,CancellationToken ct)
    {
        ...
        var detalles=await db.factura_dets.AsNoTracking().Where(x=>x.fac_numero==numero).Select(x=>new{x.pro_codigo,x.pro_codigoNavigation.pro_descripcion,x.pro_codigoNavigation.pro_modelo,x.pro_codigoNavigation.pro_unidad,x.fac_cantidad,x.fac_precio}).ToListAsync(ct);
...
""";
    ...
    public IActionResult EstiloFactura()=>Content("@page{size:A4 landscape;margin:12mm}*{box-sizing:border-box}body{font:13px Arial;color:#15233d}.sheet{max-width:1080px;margin:auto}.top{display:flex;justify-content:space-between;border-bottom:3px solid #173d75;padding-bottom:12px}.brand h1{margin:0;color:#173d75}.doc{text-align:right}.doc strong{font-size:27px}.client{display:grid;grid-template-columns:1fr 1fr;gap:7px 30px;border:1px solid #b9c7dc;padding:13px;margin:15px 0}table{width:100%;border-collapse:collapse}th{background:#173d75;color:white;padding:9px;text-align:left}td{padding:9px;border-bottom:1px solid #dce3ee}.num{text-align:right}.summary{margin:14px 0 0 auto;width:330px}.summary div{display:flex;justify-content:space-between;padding:6px 10px}.summary .total{background:#173d75;color:white;font-size:18px}.note{margin-top:28px;color:#62718b;font-size:11px}.print{position:fixed;right:18px;top:18px;padding:10px 18px;background:#176bdf;color:white;border:0;border-radius:6px}@media print{.print{display:none}}","text/css; charset=utf-8");
    ...
    public async Task<IActionResult> CrearFactura(FacturaRequest input,CancellationToken ct)
    {
        ...
        db.factura_dets.Add(new factura_det{fac_numero=numero,pro_codigo=productoMensualidad,fac_cantidad=1,fac_precio=subtotal,fac_estado="ACT"});
